...
const mainServer = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/api/clients') {
    const items = readClients()
      ...
      .map(c => ({ id: c.id, name: c.name, subtitle: c.subtitle || '', description: c.description || '', since: c.since || '', url: c.url || '', logoUrl: c.logoUrl || '', logoAlt: c.logoAlt || '' }));
...
function productContextBlock(ctx) {
  ...
  // allowBackorder is a product-level flag, it applies the same to every
  // variant, so an out-of-stock variant line needs to say so explicitly
  // rather than leaving "which options can I backorder" for the model to
  // infer from a single product-wide stock summary it can't actually derive
  // that from.
...
// ── Live catalogue tool ───────────────────────────────────────────────────────
// RAG only ever hands the model a handful of best-matching snippets, so it has
// no way to answer "how many X do you have" or a precise filtered search
...
function priceOfCatalogueRecord(rec, isService) {
  ...
  return variants.length ? Math.min(...variants.map(v => Number(v.price) || Infinity)) : (Number(rec.priceAud ?? rec.price) || 0);
}

// A literal keyword like "phone" often never appears verbatim in a listing's
// name, category, brand or description (e.g. "Samsung Galaxy S26 Ultra 5G
// 1TB" has no word "phone" anywhere in it), so a substring match against those
// fields silently returns nothing for real inventory. Matches semantically
...
async function runCatalogueSearch(args) {
  ...
  const addRec = (rec, isService) => {
    ...
    if (!Number.isFinite(maxPrice) || price <= maxPrice) {
      ...
      // A product/service word match on a shared term ("wifi") can easily pull
      // in both a real product and an unrelated service into the same result
      // list; without a clear label the model has no way to tell "sold as an
      // item" apart from "work we do" and can answer about the wrong one, or
      // deny selling a product while a same-named service sits right next to it.
      items.push({ name: rec.name, price, category: rec.category || '', options, stockNote, bulkNote, kind: isService ? 'Service' : 'Product' });
    }
  };
  ...
  return {
    ...
    items: items.slice(0, 8).map(i => {
      ...
      return `[${i.kind}] ${i.name}${priceStr}${optionsStr}${i.bulkNote || ''}${i.stockNote || ''}`;
    }),
    ...
    if (req.method === 'POST' && url.pathname === '/api/public-chat') {
      ...
      try {
        const text = await enqueueAI(() => generateVerifiedReply({
          ...
          options: { num_predict: 14000, num_ctx: 16384, temperature: 0.2 },
