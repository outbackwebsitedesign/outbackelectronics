...
function serveIndexWithOg(res, og) {
  ...
  fs.readFile(distPath, 'utf8', (err, template) => {
    ...
    res.writeHead(200, {
      ...
      'X-Content-Type-Options': 'nosniff',
      'Referrer-Policy': 'strict-origin-when-cross-origin',
      'Content-Security-Policy': "default-src 'self'; script-src 'self' 'unsafe-inline' https://*.tawk.to https://embed.tawk.to; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://*.tawk.to; img-src 'self' data: https:; font-src 'self' data: https://fonts.gstatic.com https://*.tawk.to; connect-src 'self' https://portal.outbackelectronics.com.au https://forum.outbackelectronics.com.au https://nominatim.openstreetmap.org wss://*.tawk.to https://*.tawk.to; frame-src 'self' https://www.openstreetmap.org https://*.tawk.to; frame-ancestors 'none';",
    });
    res.end(html);
  });
}
