...
**Impact:** High. **Effort:** S. **Fix applied:** `getIp()` now checks `isPrivateIp(req.socket.remoteAddress)` before trusting `CF‑Connecting‑IP` or `X‑Forwarded‑For`; if the direct connection is not from a private/loopback address those headers are ignored and the socket address is used. Locking origin ingress to Cloudflare IP ranges at the firewall is still recommended as defence‑in‑depth.
