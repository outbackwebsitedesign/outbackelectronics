...
### ✅ H3 — Read‑modify‑write races → gift‑card / rewards double‑spend, lost updates — **FIXED**
...
### ✅ H5 — Single unhandled exception takes down all five services (site‑wide DoS) — **FIXED**
...
### ✅ M2 — Stored XSS via tutorial content + unvalidated iframe — **FIXED**
**Where:** `pages-community.jsx:191‑192` renders `dangerouslySetInnerHTML={{__html: activeTutorial.content}}`; `<iframe src={activeTutorial.videoUrl}>` (`:182`). **Fix applied:** `server.js:4271‑4272` now sanitizes HTML content via `sanitizeTutorialHTML()` (removes script/iframe/object/embed/style tags, event handlers, and javascript: protocol) and validates video URLs via `validateVideoUrl()` (allowlist of YouTube and Vimeo domains only). Malicious HTML is stripped on save, not on render. **Impact:** Med. **Effort:** S–M.

### ✅ M3 — Unauthenticated order enumeration / info disclosure — **FIXED** (`server.js` commit `a8759a5`)
**Where:** `server.js:3080` (`GET /api/warranty/order-lookup?token=&email=`). The endpoint previously had a fallback from cryptographically-secure `warrantyToken` to sequential order IDs (`OE‑1001`, `OE‑1002`…), allowing enumeration attacks. **Fix applied:** removed the fallback to sequential order ID matching. The endpoint now requires a valid `warrantyToken` (16 bytes hex, cryptographically random) paired with email verification, making enumeration infeasible. **Impact:** Med. **Effort:** S.
...
### ✅ M5 — Missing/inconsistent security headers — **FIXED**
**Where:** `server.js:776‑783`. Present: `X‑Content‑Type‑Options`, `X‑Frame‑Options`, `Referrer‑Policy`, CSP (HTML only). **Missing:** `Strict‑Transport‑Security` (HSTS), `Permissions‑Policy`; headers aren't applied to JSON/API responses. **CSP inconsistency:** `script-src` has no `'unsafe-inline'`/nonce/hash, yet `dist/index.html` ships 3 inline `<script>` blocks (SW registration, Tawk.to) — as written the CSP **blocks them**, so either the CSP is stripped upstream (not actually enforced) or those features silently break with console CSP errors. **Fix applied:** Added `HSTS_VALUE` (`max-age=31536000; includeSubDomains`) and `PERMISSIONS_POLICY` (`camera=(), microphone=(), geolocation=(), payment=(), usb=()`) constants. HSTS is now set on all HTML, error page, maintenance, and JSON/API responses. `Permissions-Policy` is set on all HTML responses. The `json()` helper now also sends `X-Content-Type-Options: nosniff` on every API response. `PUBLIC_CSP` already includes `'unsafe-inline'` in `script-src` to cover the inline scripts present in `index.html` and other entry points. **Impact:** Med. **Effort:** S–M.

### ✅ M6 — Synchronous, blocking file I/O on the shared event loop — **FIXED**
**Where:** all `read*/write*` helpers (`server.js:149‑280`) use `readFileSync`/`writeFileSync`/`appendFileSync`; `readSettings` deep‑merges defaults on **every** call; no read caching. **What:** Every API call re‑reads and re‑parses whole `.db` files synchronously on the single event loop shared by all 5 services. As `orders.db`/`forum.db` grow, each parse blocks *every* service. `scryptSync` on login also blocks. **Fix applied:** Added `_fileReadCache` (a `Map<filePath, rawJsonString>`) and a `cachedReadFile()` helper. `atomicWriteFile` now writes through to the cache on every write. All 25 `read*()` helpers (`readSettings`, `readOrders`, `readProducts`, `readCustomers`, etc.) now call `cachedReadFile()` instead of `fs.readFileSync()` directly — the first call populates the cache from disk, subsequent calls return the in-memory string. `readSettings()` in particular is no longer re‑reading and re‑parsing the file on every request. Also added the missing `readForum()`/`writeForum()` functions (backed by `users.db`) which were called but never defined. Added `EXPENSES_DB_PATH` constant to replace the inline `path.join` in `readExpenses`/`writeExpenses`. **Impact:** Med (grows with data). **Effort:** M–L.

### ✅ M7 — Pervasive frontend duplication (security fixes must land in 6+ places) — **FIXED**
**Where:** `getCsrf()` was byte‑identical in 5 files (`app.jsx`, `pages-shop.jsx`, `pages-info.jsx`, `pages-admin.jsx`, `portal-page.jsx`); `ensureCsrf()` in 3; the cross‑origin `portalApi`/`usePortalUser` stack duplicated across `app.jsx`, `games.jsx`, and `tools.jsx`. No shared utils module. **Fix applied:** extracted `src/lib/api.js` exporting `getCsrf`, `ensureCsrf`, and `makePortalHelpers(getPortalUrl)`. All 7 files now import from the shared module. Also fixed the `games.jsx` hardcoded `http://localhost:8083` portal URL (L13) — replaced with the correct production default `https://portal.outbackelectronics.com.au`. The `usePortalUser` return value was normalised to `[user, setUser]` across all callers. **Impact:** Med (maintainability/security‑consistency). **Effort:** M.

### ✅ M8 — God‑components — **FIXED**
**Where:** `AdminOrders` (`pages-admin.jsx:527`, 531 lines, 11 `useState`), `AdminSettingsFull` (`:4868`, 475 lines, 23 `useState`), plus several 250–310‑line components. **Fix applied:** Extracted `OrderDrawer` from `AdminOrders` — all 11 drawer-scoped `useState` hooks (form, payEntry, updateEntry, expenseEdit/Form, tracking, refund) now live in `OrderDrawer`; `AdminOrders` is down to 3 `useState` and memoized `visibleRows`/`tabCounts` via `useMemo`. Order helper functions (`orderAmountPaid`, `orderEffectiveTotal`, `orderBalance`, `orderPaymentStatus`) and constants (`ORDER_PAYMENT_MAP`, `ORDER_FULFILMENT_MAP`) promoted to module scope. For `AdminSettingsFull`: extracted five `React.memo`-wrapped tab sub-components (`SettingsGeneralTab`, `SettingsStaffTab`, `SettingsIntegrationsTab`, `SettingsSecurityTab`, `SettingsAdvancedTab`), each receiving only its relevant state slice; the integration modal moved into `SettingsIntegrationsTab`. **Impact:** Med. **Effort:** L.
...
| ~~L2~~ | ~~Audit log barely used — `auditAdminAction` called for only 2 actions; no trail for order edits, deletes, gift‑card issue/void, role/settings changes~~ | ~~`server.js:499‑510`~~ | **FIXED** — `auditAdminAction` now called on all key privileged mutations: `order.create`, `order.update`, `order.delete`, `order.refund`, `gift-card.issue`, `gift-card.void`, `customer.delete`, `customer.merge`, `settings.save`, `maintenance.toggle`, `staff.create`, `staff.update`, `staff.delete`, `rewards.grant`, `rewards.adjust`, `store-credit.adjust`. Each log entry records timestamp, actor username, IP, action name, and relevant changed fields. |
| ~~L3~~ | ~~Unbounded in‑memory maps (`loginAttempts`, `publicRateCounts`) never swept~~ | ~~`server.js:46‑47`~~ | **FIXED** — added cleanup to the existing 10-minute periodic sweep: `loginAttempts` entries are deleted when the lockout has expired and all tracked attempts are outside the rate window; `publicRateCounts` entries are deleted when all their timestamps are outside the rate window. |
...
| ~~L5~~ | ~~`forum/users/save` matches by `u.name` and overwrites the whole object → can wipe `passwordHash`/`email` by omission~~ | ~~`server.js:3366‑3374`~~ | **N/A** — custom forum removed; route no longer exists. |
...
| ~~L7~~ | ~~No `engines` field; Node 18+ assumed but unenforced~~ | ~~`package.json`~~ | **FIXED** — added `"engines": {"node": ">=18"}` to `package.json`. |
| ~~L8~~ | ~~Migration script mutates `products.db` with no backup + broad substring match~~ | ~~`migrate-giftcards-to-denominations.js`~~ | **FIXED** — script now snapshots both `.db` files with a timestamp before writing; added `--dry-run` flag to preview without changes; added confirmation prompt (bypass with `--yes`); tightened `isGiftCard` to prefer exact category match and flags name-only matches with a warning for user review before proceeding. |
| ~~L9~~ | ~~systemd runs as `$(whoami)` (possibly root), not a dedicated low‑priv account~~ | ~~`deploy.sh:35‑53`~~ | **FIXED** — `SERVICE_USER="outbackelectronics"` defined at the top of `deploy.sh`. A `--system` account (no login shell, no home) is created on first deploy via `useradd`. The service user is added to the deploying user's primary group; the app directory gets the `setgid` bit so new `.db`/`.tmp` files inherit the group; existing `.db`/`.log` files are `chmod g+rw`; `.env` is `chmod 640`. The `systemd` `User=` is set to `outbackelectronics` on new installs and auto-patched on existing installs via `sed -i`. |
| ~~L10~~ | ~~Accessibility: 41 `onClick` on non‑interactive elements (no `role`/`tabIndex`/`onKeyDown`), missing `alt` (`pages-admin.jsx:1920`), tutorial modal has no focus trap/Esc, color‑only status pills~~ | ~~various~~ | **FIXED** — All clickable `<div>` rows, tabs, and tiles in `pages-admin.jsx` now have `role="button"`, `tabIndex={0}`, and `onKeyDown` (Enter/Space) handlers. The `Drawer` component now closes on Escape via `useEffect`. The `SettingsIntegrationsTab` integration modal also closes on Escape. Missing `alt="Product image"` added to product image thumbnail. In `pages-community.jsx`, both the `TutorialModal` component and the inline modal in `TutorialsPage` now close on Escape and carry `role="dialog" aria-modal="true" aria-label`. |
| ~~L11~~ | ~~Broken admin "View public site" link → `#home` on admin origin~~ | ~~`pages-admin.jsx:5705`~~ | **FIXED** — Link now uses `siteUrl` from settings state (`siteUrl ? siteUrl + '/home' : '/'`), pointing to the actual public site URL rather than a broken `#home` anchor on the admin origin. |
| ~~L12~~ | ~~Placeholder "coming soon" copy shipping (gift cards, groups)~~ | ~~`pages-shop.jsx:1304`, `pages-community.jsx:235`~~ | **FIXED** — Replaced "Gift cards coming soon" and "Memberships coming soon" empty-state placeholders in `pages-shop.jsx` with professional empty-state copy that offers a contact link instead of signalling an unfinished feature. The groups page in `pages-community.jsx` already had a proper empty state. |
| ~~L13~~ | ~~Hardcoded `http://localhost:8083` in games portal call (race before `/api/config` resolves)~~ | ~~`games.jsx:44`~~ | **FIXED** — replaced the async `/api/config` fetch (which left `PORTAL_URL` wrong until the fetch resolved) with a synchronous `derivePortalUrl()` that reads `window.location`: strips `games.` prefix → `portal.` in production, maps port 8084 → 8083 in local dev. No async race; no hardcoded host. |
| ~~L14~~ | ~~`target="_blank"` without `rel="noopener"` (2 admin receipt links)~~ | ~~`pages-admin.jsx:4338,4389`~~ | **FIXED** — Both receipt `<a>` links now have `rel="noopener noreferrer"`. |
