...
Port 8091 proxies an Ollama instance and adds retrieval plus tool calling. `buildRagIndex()` embeds published products, services, tutorials and policies (`getEffectivePolicies()`, audience-aware) 5 seconds after startup, caching embeddings in `rag-cache.db`; `ragSearch()` injects the top matches (min cosine similarity 0.12, a noise floor not a relevance filter) so a question works from any page, not just a product's own. `chatWithTools()` also gives the model a `search_catalogue` function it can call for availability/price/stock/count questions, answered from `products.db`/`services.db` directly (semantic-matched via the same RAG embeddings, not literal substring, since a listing's name/category often never contains the customer's exact keyword) rather than the model guessing from RAG snippets alone. Requests are queued rather than run concurrently. Without Ollama reachable, the endpoints return 503 and nothing else breaks. Ollama host/port/model names live in Settings, Integrations (the "Ollama" row) via `getOllamaConfig()`, same pattern as Stripe/Email/Icecat/AusPost; `OLLAMA_HOST`/`OLLAMA_PORT`/`AI_CHAT_MODEL`/`AI_VISION_MODEL`/`AI_EMBED_MODEL` env vars are only the fallback. It can point at another machine on the network with more RAM/a GPU, but Ollama has no built-in auth, so a remote instance must stay LAN-only or behind a VPN, never port-forwarded to the internet. `aiSystemPrompt()` includes live hours/address/contact facts and instructs the model it cannot take any action on the site (no cart/checkout/order tools exist), since it has been observed both inventing facts it was given and claiming to add items to a customer's cart.
